...
class TextDataset(Dataset):
    def __init__(self, tokenizer: PreTrainedTokenizer, args, file_path: str):
        ...
        if os.path.exists(cached_features_file) and not args.overwrite_cache:
            ...
            for i in tqdm(
                list(range(0, len(tokenized_text) - block_size + 1, block_size)), "Creating blocks"
...
class LineByLineTextDataset(Dataset):
    def __init__(self, tokenizer: PreTrainedTokenizer, args, file_path: str):
        ...
        with open(file_path, encoding="utf-8") as f:
            lines = [
                line.strip() for line in tqdm(f, "Reading lines") if (len(line.strip()) > 0 and not line.isspace())
            ]

        self.examples = tokenizer.batch_encode_plus(lines, add_special_tokens=True, max_length=args.block_size)[
            "input_ids"
        ]
...
class LineByLineTextDatasetMisspelled(Dataset):
    transforms: Tuple[str, str, str, str] = ("misspell", "uppercase", "lowercase", "remove_char")

    def __init__(self, tokenizer: PreTrainedTokenizer, args: Namespace, file_path: Union[str, Path]):
        ...
        self.examples = self._read_lines(file_path)

    def _read_lines(self, file_path: Path) -> List[str]:
        with file_path.open("r") as fhd:
            return list(tqdm(fhd, f"Reading lines from {file_path}", total=wc(file_path)))
    ...
    def _transform_with_prob(cls, text: str, transform: Callable[[str], str], prob: float) -> str:
        return transform(text) if np.random.uniform() <= prob else text

    @classmethod
    def _transform_misspell(cls, text: str) -> str:
        return make_typo(text)
    ...
    def _transform_remove_char(cls, text: str) -> str:
        char_to_remove = np.random.randint(0, len(text))
        _text = list(text)
    ...
    def convert_to_features_df(self, text: str) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        if old_text != new_text:
            logger.debug(f'Transformed: "{old_text}" -> "{new_text}"')
        ...
        inputs = torch.tensor(
            self.tokenizer.build_inputs_with_special_tokens(_inputs[: self.args.block_size - 3]), dtype=torch.long
        )
        maskable_tokens = torch.tensor(
            [False, *(_maskable_tokens[: self.args.block_size - 3]), False], dtype=torch.bool
        )
        assert inputs.shape == maskable_tokens.shape
        return inputs, maskable_tokens

    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, torch.Tensor]:
        text = self.examples[idx]
        return self.convert_to_features_df(text)


def load_and_cache_examples(args, tokenizer: PreTrainedTokenizer, evaluate: bool = False) -> Dataset:
    file_path = args.eval_data_file if evaluate else args.train_data_file
    dataset_class = LineByLineTextDatasetMisspelled if args.line_by_line else TextDataset
...
def mask_tokens(
    inputs_with_masks: Tuple[torch.Tensor, torch.Tensor], tokenizer: PreTrainedTokenizer, args, **kwargs: Any
    ...
    inputs, masks = inputs_with_masks
    labels = inputs.clone()
...
def train(args, train_dataset, model: PreTrainedModel, tokenizer: PreTrainedTokenizer) -> Tuple[int, float]:
    ...
    def collate(examples: List[Tuple[torch.Tensor, torch.Tensor]]):
        ...
        return pad_sequence(inputs, batch_first=True, padding_value=tokenizer.pad_token_id), masks

    train_sampler = RandomSampler(train_dataset) if args.local_rank == -1 else DistributedSampler(train_dataset)
    train_dataloader = DataLoader(
        train_dataset, sampler=train_sampler, batch_size=args.train_batch_size, collate_fn=collate, num_workers=4
    )
    ...
    if args.n_gpu > 1:
        loss_avger = lambda loss: loss.mean()  # mean() to average on multi-gpu parallel training
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            model.train()
            outputs = model_call(inputs, labels)
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if (
                    not args.iambot_save_every_epoch
        ...
        if args.iambot_save_every_epoch:
            save_checkpoint(args, global_step, model, tokenizer, optimizer, scheduler)
...
def evaluate(args, model: PreTrainedModel, tokenizer: PreTrainedTokenizer, prefix="") -> Dict:
    ...
    eval_dataloader = DataLoader(
        eval_dataset, sampler=eval_sampler, batch_size=args.eval_batch_size, collate_fn=collate, num_workers=4
    )
...
def iambot_train_tokenizer(args) -> Tuple[PreTrainedTokenizer, PretrainedConfig]:
    from pathlib import Path

    tokenizer_dir = Path(args.tokenizer_name)
    logger.info("Creating SentencePieceTokenizer")
    tokenizer = SentencePieceTokenizer(tokenizer_dir, **special_tokens)
...
def tokenize(line: str) -> List[str]:
    return tokenize.tokenizer.tokenize(line)


def worker_init(function, tokenizer_init_args):
    logger.warning("Initializing tokenization worker")
    function.tokenizer = SentencePieceTokenizer(**tokenizer_init_args)
...
def iambot_train_eval(args: Namespace, tokenizer: SentencePieceTokenizer) -> None:
    ...
    def load_lines(path: Union[str, Path]) -> List[str]:
        with open(path, "r") as fhd:
            return list(filter(None, map(str.strip, tqdm(fhd, f"Loading lines from {path}", wc(path)))))

    if not (Path(args.train_data_file).exists() and Path(args.eval_data_file)) or args.iambot_force_split:
        lines = load_lines(args.iambot_all_data)

        nproc = torch.multiprocessing.cpu_count()
        chunks = len(lines) // nproc
        with torch.multiprocessing.Pool(
            nproc, worker_init, (tokenize, {"model_path": args.tokenizer_name, **special_tokens})
        ) as pool:
            tokenized_sentences = tqdm(
                pool.map(tokenize, tqdm(lines, "Loading chunks"), chunks),
                "Tokenizing dataset",
                len(lines),
                maxinterval=1.0,
                miniters=1,
        ...
        tokenized_text = list(tqdm(chain.from_iterable(tokenized_sentences), "Unfolding sentences tokens"))
        del lines
        del tokenized_sentences
        logger.info(f"Tokenized text contains {len(tokenized_text)} tokens")

        # assuming that dataset has more than 1 000 000 tokens
        train_tokens = tokenized_text[:-1_000_000]
        eval_tokens = tokenized_text[-1_000_000:]

        train_data: List[str] = [
            tokenizer.convert_tokens_to_string(tokenized_text[i : i + args.block_size - 2], skip_special_tokens=True)
            for i in trange(
                0, len(train_tokens) - args.block_size + 1, args.block_size - 2, desc="Creating training examples"
            )
        ]
        eval_data: List[str] = [
            tokenizer.convert_tokens_to_string(tokenized_text[i : i + args.block_size - 2], skip_special_tokens=True)
            for i in trange(
                0, len(eval_tokens) - args.block_size + 1, args.block_size - 2, desc="Creating eval examples"
            )
        ]
...
def main():
    ...
    parser.add_argument("--iambot_all_data", type=str, help="All data needed to train tokenizer and model")
    ...
    parser.add_argument("--iambot_misspell_prob", type=float, default=0.25, help="Probability of misspell")
    parser.add_argument("--iambot_uppercase_prob", type=float, default=0.25, help="Probability of uppercase")
    parser.add_argument("--iambot_lowercase_prob", type=float, default=0.25, help="Probability of lowercase")
    parser.add_argument("--iambot_remove_char_prob", type=float, default=0.25, help="Probability of remove char")
    ...
    if args.iambot:
        ...
        tokenizer, config = iambot_train_tokenizer(args)
    else:
        if args.config_name:
